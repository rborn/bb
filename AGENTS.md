...
- To programmatically select or change the composer's model from a plugin without opening the picker UI, popping dialogs, or scraping DOM nodes, dispatch `window.dispatchEvent(new CustomEvent("bb:select-model", { detail: { model: string } }))`. `ModelReasoningPicker` listens to this event directly and updates React state silently.
